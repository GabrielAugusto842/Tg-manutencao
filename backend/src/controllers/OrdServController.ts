...
export class OrdServController {
    ...
    getOSAbertas = async (req: Request, res: Response): Promise<Response> => {
        try {
            ...
            return res.status(500).json({error: 'Erro interno ao buscar ordens de serviço em aberto!'})
        }
    }
    ...
    getById = async (req: Request, res: Response): Promise<Response> => {
        try {
            ...
            return res.status(500).json({ error: 'Erro ao buscar a ordem de serviço' });
        }
    };
    ...
    createOrdServ = async (req: Request, res: Response): Promise<Response> => {
        try {
            ...
            if (!descricao || !operacao || !idMaquina) {
                return res.status(400).json({ error: 'Campo obrigatório não preenchido!' });
            }

            const novaOS = await this.ordServRepo.createOrdServ(descricao, operacao, idMaquina);
    ...
    updateOrdServ = async (req: Request, res: Response): Promise<Response> => {
        try {
            ...
            const updated = await this.ordServRepo.updateOrdemServico(
                Number(idOrdServ), descricao, solucao || null, custo || null, idUsuario || null
            );
            if (!updated) {
                return res.status(404).json({ error: 'Ordem de serviço não encontrada' });
            }
            ...
            console.error('Erro ao atualizar informações da ordem de serviço:', error);
            return res.status(500).json({ error: 'Erro ao atualizar informações da ordem de serviço' });
        }
    };
    ...
    finalizarOrdServ = async (req: Request, res: Response): Promise<Response> => {
        try {
            ...
            const updated = await this.ordServRepo.finalizarOrdemServico(
                Number(idOrdServ), solucao, custo || null
            );
            if (!updated) {
                return res.status(404).json({ error: 'Ordem de serviço não encontrada' });
            }
            ...
            return res.status(500).json({ error: 'Erro ao finalizar ordem de serviço' });
        }
    };
    ...
    deleteOrdServ = async (req: Request, res: Response): Promise<Response> => {
        try {
            ...
            if (!deleted) {
                return res.status(404).json({ error: 'Ordem de serviço não encontrada ou já excluída' });
            }
