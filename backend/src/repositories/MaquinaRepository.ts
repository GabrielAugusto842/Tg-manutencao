...
export class MaquinaRepository {
    ...
    async getAllMaquinas(): Promise<MaquinaDetalhada[]> {
        ...
        'SELECT m.id_maquina, m.nome, m.marca, m.modelo, m.numero_serie, m.tag, m.producaoPorHora, s.id_setor, s.setor, s.descricao FROM maquina as m INNER JOIN setor as s ON m.id_setor = s.id_setor'
        );
    ...
    async findById(idMaquina: number): Promise<MaquinaDetalhada | null> {
        const [rows] = await db.execute(
        'SELECT m.id_maquina, m.nome, m.marca, m.modelo, m.numero_serie, m.tag, m.producaoPorHora, s.id_setor, s.setor, s.descricao FROM maquina as m INNER JOIN setor as s ON m.id_setor = s.id_setor WHERE s.id_setor = ?', [idMaquina]
        );
    ...
    async createMaquina(nome: string, marca: string, numeroSerie: string, modelo: string | null, 
        tag: string | null, producaoPorHora: number, idSetor: number): Promise<Maquina> {
        ...
        return new Maquina({
            id_maquina: newId, nome, marca, numero_serie: numeroSerie, modelo, tag, producaoPorHora,
            id_setor: idSetor
        });
    }
    ...
    async updateMaquina(
        ...
        tag: string | null, producaoPorHora: number, idSetor: number): Promise<boolean> {
        const [result]: any = await db.execute(
            'UPDATE maquina SET nome = ?, marca = ?, modelo = ?, numero_serie = ?, tag = ?, producaoPorHora = ?, id_setor = ? WHERE id_maquina = ?',
            [nome, marca, modelo, numeroSerie, tag, producaoPorHora, idSetor, idMaquina]
        );
